...
SegSRGAN_training_modified.py is used for training only the generator model and SegSRGAN_training_modified_full.py is used for training the full GAN model. All the inputs should be of .nii format. temporal_cropped_dgx.csv contains the path to the files which is used for training. 
