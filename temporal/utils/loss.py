...
def gradient_penalty_loss(y_true, y_pred, averaged_samples):
    if(1):
        ...
        print ("INSIDE GP FUNCTION BEGINNING")
        ...
        print ("INSIDE GP FUNCTION ENDING")
...
def Dice_loss(y_true, y_pred):
    ...
    smooth = 1
    print ("INSIDE DICE LOSS CHECKING DATA TYPE", type(y_true))

    '''intersection = 2 * K.sum(y_true[:,:,1:] * y_pred[:,:,1:],axis=(0,1,3,4,5))+ smooth
    denominator = K.sum( y_pred[:,:,1:], axis=(0,1,3,4,5)) +  K.sum(y_true[:,:,1:],axis=(0,1,3,4,5)) + smooth
    ...
    for i in range(y_true.shape[1]):
        intersection = 2 * K.sum(y_true[:, i, 1:] * y_pred[:, i, 1:],axis=(0,2,3,4))+ smooth
        denominator = K.sum(y_pred[:,i, 1:], axis=(0,2,3,4)) +  K.sum(y_true[:, i, 1:],axis=(0,2,3,4)) + smooth
        dice_loss_per_class = 1 - intersection / denominator
        dice_loss = K.mean(dice_loss_per_class)
        tot_loss += dice_loss
    dice_loss = tot_loss / y_true.shape[1]
    loss =  dice_loss + SR_loss
    #print ("INSIDE Dice_Loss FUNCION, LOSS VALUE", loss)
    return loss
